...
struct QualityChecker {
  ...
  std::array<int, getNumDE()> chMap;
  std::array<std::pair<int, int>, getNumDE()> deMap;
  std::array<o2::quality_control::core::Quality, getNumDE()> quality;
};
...
struct FecId {
  ...
  static int max() { return ((sFeeNum * sLinkNum * sDsNum) - 1); }
