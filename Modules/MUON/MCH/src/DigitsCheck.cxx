...
void DigitsCheck::configure()
{
  ...
  if (auto param = mCustomParameters.find("BadChanFracHistName"); param != mCustomParameters.end()) {
    mBadChanFracHistName = param->second;
  }
  ...
  if (auto param = mCustomParameters.find("MaxEmptyFraction"); param != mCustomParameters.end()) {
    mMaxEmptyFraction = std::stof(param->second);
  }
...
int DigitsCheck::checkPadMapping(uint16_t feeId, uint8_t linkId, uint8_t eLinkId, o2::mch::raw::DualSampaChannelId channel)
{
  uint16_t solarId = -1;
  int deId = -1;
  int dsIddet = -1;
...
Quality DigitsCheck::checkFecRate(MergeableTH2Ratio* hr)
{
  ...
  for (int i = 1; i <= nbinsx; i++) {
    ...
    for (int j = 1; j <= nbinsy; j++) {
      ...
      npads += 1;
      if (occupancy >= mMinRate && occupancy <= mMaxRate) {
        ngood += 1;
      }
    }
  }
...
{
  return checkPlot(h, [&](double val) -> bool { return (val <= mMaxEmptyFraction); });
}
...
{
  ILOG(Info, Devel) << "Entered DigitsCheck::check" << ENDM;
  ...
  for (auto& [moName, mo] : *moMap) {
    ...
    if (matchHistName(mo->getName(), mBadChanFracHistName)) {
      TH1F* h = getHisto<TH1F>(mo);
      if (h) {
...
void DigitsCheck::beautify(std::shared_ptr<MonitorObject> mo, Quality checkResult)
{
  ...
  if (mo->getName().find("BadChannelsFraction") != std::string::npos) {
    TH1F* h = getHisto<TH1F>(mo);
    if (!h) {
      return;
    }
    ...
    // draw horizontal limits
    TLine* l = new TLine(0, mMaxEmptyFraction, h->GetXaxis()->GetXmax(), mMaxEmptyFraction);
    l->SetLineColor(kRed);
    l->SetLineStyle(kDashed);
    h->GetListOfFunctions()->Add(l);
  ...
  if (mo->getName().find("QualityFlagPerDE") != std::string::npos) {
    ...
    for (int deId = 0; deId < mQualityChecker.quality.size(); deId++) {
      float ybin = 0;
      if (mQualityChecker.quality[deId] == Quality::Good) {
        ybin = 1;
      }
      if (mQualityChecker.quality[deId] == Quality::Medium) {
        ybin = 2;
      }
      if (mQualityChecker.quality[deId] == Quality::Bad) {
        ybin = 3;
      }
