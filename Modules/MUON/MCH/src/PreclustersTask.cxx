...
bool PreclustersTask::getFecChannel(int deId, int padId, int& fecId, int& channel)
{
  ...
  fecId = FecId(feeId, linkId, dsAddr).mFecId; // feeId * PreclustersTask::sMaxLinkId * PreclustersTask::sMaxDsId + (linkId % PreclustersTask::sMaxLinkId) * PreclustersTask::sMaxDsId + dsAddr;
...
void PreclustersTask::plotPrecluster(const o2::mch::PreCluster& preCluster, gsl::span<const o2::mch::Digit> digits)
{
  ...
  if (hasSignal[0] || hasSignal[1]) {
    ...
    mHistogramClusterSize->Fill(getDEindex(detid) * 3, multiplicity[0]);
    mHistogramClusterSize->Fill(getDEindex(detid) * 3 + 1, multiplicity[1]);
    mHistogramClusterSize->Fill(getDEindex(detid) * 3 + 2, multiplicity[0] + multiplicity[1]);

    // total cluster charge
    float chargeTot = chargeSum[0] + chargeSum[1];
    mHistogramClusterCharge->Fill(getDEindex(detid), chargeTot);
  }
}
