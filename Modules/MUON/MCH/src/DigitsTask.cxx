...
void DigitsTask::initialize(o2::framework::InitContext& /*ctx*/)
{
  ...
  mHistogramDigitsOrbitInTF = std::make_unique<TH2F>("DigitOrbitInTF", "Digit orbits vs DS Id", nElecXbins, 0, nElecXbins, 768, -384, 384);
  publishObject(mHistogramDigitsOrbitInTF.get(), "colz", true, false);

  mHistogramDigitsSignalOrbitInTF = std::make_unique<TH2F>("DigitSignalOrbitInTF", "Digit orbits vs DS Id (signal)", nElecXbins, 0, nElecXbins, 768, -384, 384);
  publishObject(mHistogramDigitsSignalOrbitInTF.get(), "colz", true, false);

  mHistogramDigitsBcInOrbit = std::make_unique<TH2F>("Expert/DigitsBcInOrbit", "Digit BC vs DS Id", nElecXbins, 0, nElecXbins, 3600, 0, 3600);
...
void DigitsTask::plotDigit(const o2::mch::Digit& digit)
{
  ...
  int xbin = FecId(feeId, linkId, dsAddr).mFecId + 1;
  int ybin = channel + 1;
  ...
  if (tfTime != o2::mch::raw::DataDecoder::tfTimeInvalid) {
    ...
    if (orbit < -50 && orbit > -130) {
      int board = linkId * 40 + dsAddr;
      if ((feeId % 2) != 0) {
        board += 40 * 12;
      }
    }
  ...
  if (isSignal) {
    mHistogramDigitsSignalOrbitInTF->Fill(xbin - 0.5, orbit);
  }
}
  ...
  for (uint16_t feeId = 0; feeId < FecId::sFeeNum; feeId++) {
    ...
    for (uint8_t linkId = 0; linkId < FecId::sLinkNum; linkId++) {
      ...
      for (uint8_t dsAddr = 0; dsAddr < FecId::sDsNum; dsAddr++) {
        ...
        int xbin = FecId(feeId, linkId, dsAddr).mFecId + 1; // feeId * DigitsTask::sMaxLinkId * DigitsTask::sMaxDsId + (linkId % DigitsTask::sMaxLinkId) * DigitsTask::sMaxDsId + dsAddr + 1;
