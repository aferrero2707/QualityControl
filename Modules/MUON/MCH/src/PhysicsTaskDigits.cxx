...
void PhysicsTaskDigits::plotDigit(const o2::mch::Digit& digit)
{
  ...
  std::optional<DsElecId> dsElecId = mDet2ElecMapper(DsDetId{de, padid});
  ...
  // fill 2D histogram with fired pads distribution
    ...
    for (int by = 0; by < NYbins; by++) {
      ...
      for (int bx = 0; bx < NXbins; bx++) {
        ...
        if(segment.findPadPairByPosition(x, y, bpad, nbpad)){
          ...
          //int chan_addr_boucle = segment.padDualSampaChannel(padid_boucle);
          std::optional<DsElecId> dsElecId = mDet2ElecMapper(DsDetId{de, padid_boucle});
          if (dsElecId.has_value()) {
            solar_id_boucle = dsElecId->solarId();
            //ds_addr_boucle = dsElecId->elinkId();

            // get the feeId/linkId associated to this bin
            //std::optional<FeeLinkId> feeLinkId = mSolar2FeeLinkMapper(solar_id);
            //if (feeLinkId.has_value()) {
            //  fee_id_boucle = feeLinkId->feeId();
            //  linkid_boucle = feeLinkId->linkId();
            //}
          }
