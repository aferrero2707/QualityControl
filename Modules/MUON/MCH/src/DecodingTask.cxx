...
DecodingTask::~DecodingTask() = default;

static void setXAxisLabels(TH1F* hErrors)
{
...
void DecodingTask::createErrorHistos()
{
  ...
  mHistogramErrorsFEC = std::make_unique<MergeableTH2Ratio>("DecodingErrors/DecodingErrorsFEC", "Error Code vs. FEC ID", nElecXbins, 0, nElecXbins, getErrorCodesSize(), 0, getErrorCodesSize());
  {
  ...
  mHistogramErrorsPerDE = std::make_unique<MergeableTH2Ratio>("DecodingErrors/DecodingErrorsPerDE",
                                                              "Error Type vs. DE ID",
                                                              getNumDE(), 0, getNumDE(),
                                                              getErrorCodesSize(), 0, getErrorCodesSize());
  setYAxisLabels(mHistogramErrorsPerDE.get());
  publishObject(mHistogramErrorsPerDE.get(), "colz", "gridy logz", false, false);

  // Number of decoding errors, grouped by chamber ID and normalized to the number of processed TF
  mHistogramErrorsPerChamber = std::make_unique<MergeableTH2Ratio>("DecodingErrors/DecodingErrorsPerChamber",
                                                                   "Error Type vs. Chamber Number", 10, 1, 11,
                                                                   getErrorCodesSize(), 0, getErrorCodesSize());
  setXAxisLabels(mHistogramErrorsPerChamber.get());
  setYAxisLabels(mHistogramErrorsPerChamber.get());
  publishObject(mHistogramErrorsPerChamber.get(), "colz", "gridx gridy logz", false, false);

  // Number of decoding errors, grouped by FEE ID and normalized to the number of processed TF
  mHistogramErrorsPerFeeId = std::make_unique<MergeableTH2Ratio>("DecodingErrors/DecodingErrorsPerFeeId",
                                                                 " Error Type vs. FEE ID", 64, 0, 64,
                                                                 getErrorCodesSize(), 0, getErrorCodesSize());
  setYAxisLabels(mHistogramErrorsPerFeeId.get());
  publishObject(mHistogramErrorsPerFeeId.get(), "colz", "gridy logz", false, false);
}
...
void DecodingTask::createHeartBeatHistos()
{
  ...
  mHistogramHBTimeFEC = std::make_unique<MergeableTH2Ratio>("SyncErrors/HBTimeFEC", "HB time vs. FEC ID", nElecXbins, 0, nElecXbins, 40, mHBExpectedBc - 20, mHBExpectedBc + 20);
  publishObject(mHistogramHBTimeFEC.get(), "colz", "logz", false, false);

  uint64_t max = ((static_cast<uint64_t>(0x100000) / 100) + 1) * 100;
  mHistogramHBCoarseTimeFEC = std::make_unique<MergeableTH2Ratio>("SyncErrors/HBCoarseTimeFEC", "HB time vs. FEC ID (coarse)", 64 * 12 * 40, 0, 64 * 12 * 40, 100, 0, max);
  publishObject(mHistogramHBCoarseTimeFEC.get(), "colz", "", false, false);

  mSyncStatusFEC = std::make_unique<TH2F>("SyncErrors/SyncStatusFEC", "Heart-beat status vs. FEC ID", 64 * 12 * 40, 0, 64 * 12 * 40, 3, 0, 3);
  ...
  publishObject(mSyncStatusFEC.get(), "col", "gridy", false, false);

  mHistogramSynchErrorsPerDE = std::make_unique<MergeableTH1Ratio>("SyncErrors/SynchErrorsPerDE", "Out-of-sync boards fraction per DE", getNumDE(), 0, getNumDE());
  publishObject(mHistogramSynchErrorsPerDE.get(), "hist", "", false, false);

  mHistogramSynchErrorsPerChamber = std::make_unique<MergeableTH1Ratio>("SyncErrors/SynchErrorsPerChamber", "Out-of-sync boards fraction per chamber", 10, 0, 10);
  setXAxisLabels(mHistogramSynchErrorsPerChamber.get());
  publishObject(mHistogramSynchErrorsPerChamber.get(), "hist", "", false, false);
}
...
void DecodingTask::plotError(int solarId, int dsAddr, int chip, uint32_t error)
{
  int feeId{ -1 };
  int linkId{ -1 };
  int deId{ -1 };
  ...
  try {
    ...
    ILOG(Warning, Devel) << e.what() << "  SOLAR" << solarId << "  DS" << dsAddr << "  CHIP" << chip << "  ERROR " << error << ENDM;
  }

  if (feeId < 0 || deId < 0 || chamberId < 0) {
    return;
  }

  auto fecId = FecId(feeId, linkId, dsAddr).mFecId;
  uint32_t errMask = 1;
  for (int i = 0; i < getErrorCodesSize(); i++) {
    // Fill the error histogram if the i-th bin is set in the error word
    if ((error & errMask) != 0) {
      mHistogramErrorsFEC->getNum()->Fill(fecId, 0.5 + i);
      mHistogramErrorsPerDE->getNum()->Fill(deId, 0.5 + i);
      mHistogramErrorsPerChamber->getNum()->Fill(chamberId, 0.5 + i);
      mHistogramErrorsPerFeeId->getNum()->Fill(feeId, 0.5 + i);
    }
    errMask <<= 1;
  }
}
...
void DecodingTask::plotHBPacket(int solarId, int dsAddr, int chip, uint32_t bc)
{
  int feeId{ -1 };
  std::optional<FeeLinkId> feeLinkId = mSolar2Fee(solarId);
  if (feeLinkId) {
...
void DecodingTask::monitorData(o2::framework::ProcessingContext& ctx)
{
  ...
  updateTFcount(mHistogramErrorsPerDE.get(), nTF);
...
int DecodingTask::getDeId(uint16_t feeId, uint8_t linkId, uint8_t eLinkId)
{
  uint16_t solarId = -1;
  int deId = -1;
  int dsIddet = -1;
...
void DecodingTask::checkSyncErrors()
{
