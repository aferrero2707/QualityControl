...
QualityChecker::QualityChecker()
{
  for (auto de : o2::mch::constants::deIdsForAllMCH) {
    ...
    if (idx >= 0) {
      chMap[idx] = getChamberIndex(de);
      deMap[idx] = getDEindexInChamber(de);
    }
  }
...
{
  std::fill(quality.begin(), quality.end(), Quality::Null);
}

void QualityChecker::addCheckResult(gsl::span<Quality> result)
{
  if (quality.size() > result.size()) {
    return;
  }

  for (int i = 0; i < quality.size(); i++) {
    if ((quality[i] == Quality::Null) || (result[i] == Quality::Bad)) {
      quality[i] = result[i];
    }
  }
}
...
Quality QualityChecker::getQuality()
{
  ...
  for (int i = 0; i < quality.size(); i++) {
    ...
    if (quality[i] != Quality::Bad) {
      continue;
    }
    auto chamber1 = chMap[i];
    ...
    for (int j = i + 1; j < quality.size(); j++) {
      // if detector is OK, skip it
      if (quality[j] != Quality::Bad) {
        continue;
      }
  ...
  if (nBadDE[0][0] > maxBadST12 || nBadDE[0][1] > maxBadST12) {
    result = Quality::Bad;
  }

  if (nBadDE[1][0] > maxBadST345 || nBadDE[1][1] > maxBadST345) {
    result = Quality::Bad;
  }
